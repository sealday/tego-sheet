...
describe('clear contents', () => {
  it('clears text and cached values across a range while preserving cell metadata', () => {
    ...
    expect(controller.getValue()[0]).toMatchObject({
      rows: { 0: { cells: {
        0: { text: '', style: 0, vendorCell: { keep: true } },
        1: { text: '', editable: true },
        2: { text: 'outside', value: 3 },
      } } },
    });
  ...
  it('keeps an already empty range silent and history-free', () => {
    const controller = new WorkbookController({
      rows: { len: 2, 0: { cells: { 0: { text: '', style: 0 } } } },
      cols: { len: 2 },
      styles: [{ color: '#f00' }],
    });
    const before = controller.getValue();
