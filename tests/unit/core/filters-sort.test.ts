...
describe('filter and corrected sorting transforms', () => {
  ...
  it('@parity:tools.sort-total-order keeps empties last descending with stable ties', () => {
    ...
    expect(sortValues(['2', 2, '02', true, 'TRUE', null, undefined], 'asc', locale)).toEqual([
      '2',
      2,
      '02',
      true,
  ...
  it('@parity:correction.sort-rendered-values sorts selected-column rendered values, not row indexes', () => {
    expect(
      sortRows(sortSheet, 1, 'asc', locale, {
        start: { row: 0, column: 0 },
        end: { row: 3, column: 1 },
      }),
    ).toEqual([2, 3, 1]);
  ...
  it('persists set/clear filter and sort commands with one deterministic history entry each', () => {
    ...
    ).toMatchObject({ status: 'committed', commit: { change: { kind: 'filter' } } });
    expect(
      controller.dispatch(
        {
          type: 'set-filter',
    ...
    expect(
      controller.dispatch({ type: 'sort', sheet, column: 1, order: 'desc' }, 'toolbar').status,
    ).toBe('committed');
    ...
    expect(controller.dispatch({ type: 'clear-filter', sheet }, 'toolbar').status).toBe(
      'committed',
    );
  ...
  it('rejects 250001-row filter and sort workloads before allocation or controller mutation', () => {
    ...
    expect(() =>
      controller.dispatch({ type: 'sort', sheet, column: 0, order: 'asc' }, 'toolbar'),
    ).toThrowError(expect.objectContaining({ code: 'INVALID_COMMAND' }));
  ...
  it('budgets candidate-set construction before pure or controller filter work', () => {
    const boundaryRange = {
      start: { row: 0, column: 0 },
      end: { row: 125_000, column: 0 },
    };
    expect(() =>
      assertFilterResourceLimit(boundaryRange, [
        {
          ci: 0,
          operator: 'in',
          value: Array<string>(125_000).fill('keep'),
        },
      ]),
    ).not.toThrow();
    expect(() =>
      assertFilterResourceLimit(boundaryRange, [
        {
          ci: 0,
          operator: 'in',
          value: Array<string>(125_001).fill('keep'),
        },
      ]),
    ).toThrow(RangeError);
    ...
    expect(() =>
      controller.dispatch(
        {
          type: 'set-filter',
          selection: { sheet, range, active: range.start },
  ...
  it('shares one formula-reference budget across every sort key and rejects before mutation', () => {
    ...
    const before = controller.getValue();
    expect(() =>
      controller.dispatch({ type: 'sort', sheet, column: 1, order: 'asc' }, 'toolbar'),
    ).toThrowError(expect.objectContaining({ code: 'INVALID_COMMAND' }));
    expect(controller.getValue()).toEqual(before);
    expect(controller.historySize.undo).toBe(0);
    ...
    expect(() =>
      sortRows(sharedDependency, 0, 'asc', locale, {
        start: { row: 0, column: 0 },
        end: { row: 2, column: 0 },
      }),
    ).toThrow(RangeError);
