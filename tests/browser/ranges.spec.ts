...
test('@parity:ranges.drag-fill exercises merge, format painting, clearing, and autofill through browser UI', async ({
  page,
}) => {
  ...
  let value = (await capture(page)) as Array<{
    ...
    rows?: Record<
      string,
      { cells?: Record<string, { merge?: number[]; style?: number; text?: string }> }
    >;
  }>;
