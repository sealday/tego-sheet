...
test('@parity:editing.ime-input keeps composition in the editor until explicit commit', async ({
  page,
}) => {
  ...
  await editor.evaluate((element) => {
    ...
    textarea.dispatchEvent(
      new InputEvent('input', { bubbles: true, data: '漢字', inputType: 'insertCompositionText' }),
    );
  ...
  const readOnlyValue = (await capture(page)) as Array<{
    rows?: Record<string, { cells?: Record<string, { text?: string }> }>;
  }>;
