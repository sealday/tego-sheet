  ...
  const commitEditor = useCallback(
    (
      ...
      const proposedTarget =
        selectionAfterCommit ?? (move === undefined ? null : (engine?.nextSelection(move) ?? null));
      const nextTarget =
        proposedTarget !== null &&
        !sameSelection(
          engine?.publicSelection() ?? proposedTarget.selection,
          proposedTarget.selection,
        )
          ? proposedTarget
          : null;
  ...
  const requestEdit = useCallback(
    (point: CellPoint, initialText: string | undefined, source: ChangeSource) => {
      ...
      replaceEditor({
        ...
        onCommit: (move) => {
          void commitEditor(undefined, move);
        },
      });
    },
