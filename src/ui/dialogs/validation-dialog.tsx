...
export function ValidationDialog(props: {
    ...
    type === 'list' && operator !== ''
      ...
      : type === 'list' && trimmedValue === ''
        ...
        : (operator === 'be' || operator === 'nbe') &&
          ...
          ? props.t(
              'validation.rangeValuesRequired',
              'Enter exactly two nonblank comma-separated values.',
            )
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={props.t('validation.title', 'Data validation')}
      className="tego-sheet__dialog"
    >
      <label>
        {props.t('validation.type', 'Type')}
        <select
          name="type"
          value={type}
          onChange={(event) => setType(event.target.value as ValidationType)}
        >
          {validationTypes.map(([item, label]) => (
            <option key={item} value={item}>
              {props.t(`validation.types.${item}`, label)}
            </option>
          ))}
        </select>
      </label>
      <label>
        {props.t('validation.operator', 'Operator')}
        <select
          value={operator}
          onChange={(event) => setOperator(event.target.value as ValidationOperator | '')}
        >
          <option value="">{props.t('validation.none', 'None')}</option>
          {validationOperators.map(([item, label]) => (
            <option key={item} value={item}>
              {props.t(`validation.operators.${item}`, label)}
            </option>
          ))}
        </select>
      </label>
      <label>
        {props.t('validation.value', 'Value')}
        <input
          aria-describedby="tego-sheet-validation-status"
          value={value}
          onChange={(event) => setValue(event.currentTarget.value)}
        />
      </label>
      <p id="tego-sheet-validation-status" role="status" aria-live="polite">
        {validationMessage}
      </p>
      <label>
        <input
      ...
      <button type="button" onClick={props.onRemove}>
        {props.t('validation.remove', 'Remove validation')}
      </button>
