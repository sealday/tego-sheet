...
function translatedObjectAnchor(
  object: SheetObject,
  generated: DocumentCellRange,
  source: DocumentCellRange,
): SheetObject['anchor'] {
  if (object.anchor.type === 'absolute') return object.anchor;
  const rowDelta = generated.start.row - source.start.row;
  const columnDelta = generated.start.column - source.start.column;
  const point = <
    T extends { readonly sheetId: string; readonly row: number; readonly column: number },
  >(
    value: T,
  ): T => ({
    ...value,
    sheetId: generated.sheetId,
    row: value.row + rowDelta,
    column: value.column + columnDelta,
  });
  return object.anchor.type === 'one-cell'
    ? { ...object.anchor, cell: point(object.anchor.cell) }
    : {
        ...object.anchor,
        from: point(object.anchor.from),
        to: point(object.anchor.to),
      };
}
...
function materializeRepeatedObjects(
  ...
  for (const binding of compiled.ir.bindings) {
    ...
    const bindingMappings = mappings.filter(({ bindingId }) => bindingId === binding.id);
    const repeatedSources = binding.objects.map((reference) => ({
      reference,
      object: sourceSheet?.objects.find(({ id }) => id === reference.id),
    }));
    const sourceObjects = repeatedSources.flatMap(({ reference, object }) =>
      object === undefined ? [] : [{ reference, object }],
    );
    const sourceIds = new Set(sourceObjects.map(({ object }) => String(object.id)));
    const affectedSheetIds = new Set([
      String(binding.range.sheetId),
      ...bindingMappings.map(({ generated }) => String(generated.sheetId)),
    ]);
    const existingIds = new Set(
      nextDocument.workbook.sheets.flatMap((sheet) =>
        sheet.objects
          .filter(({ id }) => !affectedSheetIds.has(String(sheet.id)) || !sourceIds.has(String(id)))
          .map(({ id }) => String(id)),
      ),
    );
    const generatedBySheet = new Map<string, SheetObject[]>();
    for (const mapping of bindingMappings) {
      ...
      for (const { reference, object } of repeatedSources) {
        ...
        const generatedObject = {
          ...
          anchor: translatedObjectAnchor(object, mapping.generated, mapping.source),
        } as SheetObject;
    ...
    if (sourceObjects.length > 0) {
      nextDocument = freeze({
        ...nextDocument,
        workbook: {
          ...nextDocument.workbook,
          sheets: nextDocument.workbook.sheets.map((sheet) => ({
            ...sheet,
            objects: [
              ...sheet.objects.filter(
                ({ id }) => !affectedSheetIds.has(String(sheet.id)) || !sourceIds.has(String(id)),
              ),
              ...(generatedBySheet.get(String(sheet.id)) ?? []),
            ].sort(
              (left, right) =>
                left.zIndex - right.zIndex || String(left.id).localeCompare(String(right.id)),
            ),
          })),
        },
      });
  ...
  return { document: nextDocument, mappings: objectMappings };
}
  ...
  for (const root of [...roots].sort(
    ...
    const renderNode = (
      ...
      if (
        ...
        return { cells: [], rows: [], merges: [], height: 0 };
      }
      ...
      for (const [index, item] of copies.entries()) {
        ...
        if (
          binding.type === 'repeat-rows' &&
          binding.pageBreak === 'before-each-item' &&
          index > 0
        ) {
    ...
    const renderRange = (
      ...
      while (sourceRow <= range.end.row) {
        ...
        if (child !== undefined && child.range.start.row === sourceRow) {
          const childAxis = axis(child);
          const fragment =
            childAxis === 'horizontal'
    ...
    sheets[sheetIndex] = {
      ...
      cells: [
        ...sourceSheet.cells.filter(({ row, column }) =>
          rootAxis === 'horizontal' ? column < root.range.start.column : row < root.range.start.row,
        ),
        ...fragment.cells,
        ...sourceSheet.cells
          .filter(({ row, column }) =>
            rootAxis === 'horizontal' ? column > root.range.end.column : row > root.range.end.row,
          )
          .map((entry) =>
            rootAxis === 'horizontal'
      ...
      merges: [
        ...sourceSheet.merges
          ...
          .map((merge) =>
            rootAxis === 'horizontal' && merge.start.column > root.range.end.column
              ? {
                  start: { ...merge.start, column: merge.start.column + columnDelta },
                  end: { ...merge.end, column: merge.end.column + columnDelta },
                }
              : rootAxis !== 'horizontal' && merge.start.row > root.range.end.row
                ? {
...
function expandNestedColumns(
  ...
  for (const root of [...roots].sort(
    ...
    const estimateNode = (
      ...
      let width = 0;
      const childRanges = node.children.map(({ range }) => range);
      const ownColumns = Array.from(
        { length: node.range.end.column - node.range.start.column + 1 },
        (_, offset) => node.range.start.column + offset,
      ).filter(
        (column) =>
          !childRanges.some((range) => column >= range.start.column && column <= range.end.column),
      );
      for (const [index, item] of copies.entries()) {
        const scope: Scope = {
          root: data,
        ...
        width += ownColumns.length;
        cells += ownColumns.reduce((count, column) => {
          const sourceCount = source.cells.filter(
            ({ column: candidate }) => candidate === column,
          ).length;
          const blankTargets = values.filter(
            ({ target }) =>
              target.sheetId === source.id &&
              target.column === column &&
              !source.cells.some(
                (entry) => entry.row === target.row && entry.column === target.column,
              ),
          ).length;
          return count + sourceCount + blankTargets;
        }, 0);
        for (const child of node.children) {
          const childEstimate = estimateNode(child, scope);
          width += childEstimate.width;
          cells += childEstimate.cells;
        }
      }
      return { cells, width };
    };
    ...
    const estimate = estimateNode(root, { root: data });
    const replaced = source.cells.filter(
      ({ column }) => column >= root.range.start.column && column <= root.range.end.column,
    ).length;
    if (
      source.cells.length - replaced + estimate.cells > limits.maxExpandedCells ||
      estimate.width > (limits.maxExpandedColumns ?? 16_384)
    ) {
      return {
        diagnostics: [
          error(
...
export function expandAdvancedTemplate(
  ...
  for (const { binding, items } of [...estimates].sort(
    ...
    if (
      ...
      (binding.type === 'repeat-rows' && binding.pageBreak === 'before-each-item')
    ) {
